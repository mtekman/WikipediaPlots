...
** Second attempt using wide format
   ...
   #+begin_src R
     ...
     commonfancyplot <- function(topbreaks, topscale, botbreaks, botscale){

       f1 <- fancyplot(sub.ev.a1900, "Day", NULL,
                       #"Days with Significant Years > 1900 (Events)",
                       topbreaks, scale=topscale, color="darkgreen")
       f2 <- fancyplot(sub.ev.a1900, "Month", NULL,
                       #"Months with Significant Years > 1900 (Events)",
                       topbreaks, scale=topscale, color="darkgreen")
       f3 <- fancyplot(sub.bd.a1900, "Day", NULL,
                       #"Days with Significant Years > 1900 (Births,Deaths)",
                       botbreaks, scale=botscale, color="darkblue")
       f4 <- fancyplot(sub.bd.a1900, "Month", NULL,
       ...
       grid.arrange(pp1,pp2,pp3,pp4,ncol=2) + theme(axis.title.y = "Years with Events Observed")
     }

     commonfancyplot(seq(0,1000,200), 0.03, seq(0,8000,2000), 0.003)
   #+end_src
