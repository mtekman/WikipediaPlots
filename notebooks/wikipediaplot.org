...
** Second attempt using wide format
   ...
   #+begin_src R
     ...
     fancyplot <- function(data, type, title, breakers, plot.type="geom_boxplot", scale=0.05, color=color){
       plotit(data, type) +
         ggtitle(title) +
         get(plot.type)(fill=color, color="grey", notch=T) +
     ...
     commonfancyplot <- function(topbreaks, topscale, botbreaks, botscale){
       ...
       grid.arrange(
         arrangeGrob(pp1,pp2,pp3,pp4,ncol=2,
                     top = textGrob("Significant Years (1900 - Now)", vjust = 1,
